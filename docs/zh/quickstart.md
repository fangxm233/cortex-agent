...
[桌面应用](desktop-app.md)可以替你安装并配置服务端：在首屏选择**在本机安装并启动**，它会安装软件包、执行本指南中同样的初始化、启动 daemon 并完成连接。Node.js 仍是前置条件，Slack 与飞书则被完全跳过——应用本身就是交互界面。如果你还想让服务端接入聊天平台，请继续阅读本指南。
