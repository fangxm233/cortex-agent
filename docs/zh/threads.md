...
这些文件可以直接手工编辑，热重载会自动生效。也可以在桌面端的 **设置 → 线程模板** 中编辑——后者更稳妥，因为它在写入前先做校验。

面板是主从式编辑器：左侧列出全部 agent、template 和 shell，当前校验不通过的条目带标记；右侧是该实体的原始 JSON，外加两个只读页签。**校验** 列出错误与警告，并标注各自来自哪个字段；**引用** 显示磁盘上的文件路径、哪些模板依赖该实体、有多少正在运行的线程使用它、以及有多少未完成的任务指名它。

校验在保存时和手动触发时运行，判定依据是这次保存之后会产生的世界，而非它所替换的那个——因此一个模板可以引用同一批编辑中正在新建的 agent。错误会阻止写入；警告（多数是未识别的字段）会报告但放行。最关键的是影响面检查：保存一个 agent 会重新校验所有使用它的模板，正是这一步拦下了那类会让运行中的线程无处可走的编辑。因为执行器在**每一步**都重新读取转换规则与提示，坏编辑不会在启动时大声失败——它会让线程在半途悄悄卡住。

写入路径上有三道保护。对有活跃线程的实体保存需要二次确认，因为那些线程会在下一步取到改动。只要还有其他模板引用该实体，删除会被直接拒绝。保存会带上编辑器载入时那份内容的哈希，因此基于过期视图做出的编辑——别人的保存，或你自己经热重载落地的手工改动——会被判为冲突而拒绝，而不是无声覆盖。

不提供重命名：文件名**就是**身份，重命名会让指向旧名的所有引用变成孤儿。请复制为新名再删除旧的，这样中间必须显式更新依赖它的模板。

实体按来源标注。**stock** 表示与 Cortex 发货文件逐字节一致，**modified** 表示发货文件被你改过，**custom** 表示完全由你创建。编辑 stock 实体会永久分叉——默认值合并是 copy-if-missing，所以你的版本永远不会被升级覆盖，同样也永远不会拿到发货版本的改进。配置目录是纯 JSON，值得纳入 git 管理；那才是撤销手段，而不是备份文件。
