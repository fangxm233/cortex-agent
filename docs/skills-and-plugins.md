...
Remote servers require HTTPS except for loopback HTTP. URLs containing credentials or fragments are rejected, and configured headers are never treated as a portable secret mechanism. Both backends reject every remote MCP redirect: PI applies the shared redirect-rejecting fetch directly, while Claude reaches each remote server through a local stdio proxy with the same policy. This prevents configured headers and request bodies from being replayed to a redirect target (`agent-server/src/agent-adapter/mcp-remote-fetch.ts:20-38`; `agent-server/src/agent-adapter/claude/remote-mcp-proxy.ts:48-62`). The Settings UI shows only the remote origin and header names; for stdio it shows only the executable basename, argument count, and environment key names. Environment and header values are not returned to the browser.
...
Cortex reloads the selected catalog at spawn time. Legacy plugin directories retain their existing backend path behavior. Portable skill support trees are containment-checked, content-hashed, and copied into backend-specific private projections. Claude receives its compatibility projection through `--plugin-dir`; PI receives only the skill directories inside its private projection, as additional skill paths on its in-process session. Neither backend receives a portable skill path that points directly into the mutable installed package (`agent-server/src/domain/plugins/skill-projection.ts:23-151`; `agent-server/src/domain/plugins/runtime.ts:346-494,629-632`).

Portable MCP declarations pass through one backend-neutral normalization layer. Claude receives a private supplemental configuration; remote entries in that file are local stdio proxies whose separate private files hold the remote URL and headers. PI's MCP bridge receives the normalized server entries on the session request and opens each connection itself (stdio, Streamable HTTP, or SSE). Components fail at their declared dependency boundary: an invalid or unmaterializable skill is omitted without removing sibling skills or valid MCP; unavailable plugin-scoped `PLUGIN_DATA` omits the stdio servers that share it while preserving remote MCP and skills; and remote/stdio processes connect independently from sibling processes and bundled tools (`agent-server/src/domain/plugins/runtime.ts:186-205,346-494`; `agent-server/src/agent-adapter/claude/mcp-config.ts:105-164`; `agent-server/src/agent-adapter/pi/mcp-bridge.ts`).
...
Portable MCP is suppressed for the explicit `none` composition and for the restricted subagent surface. This keeps plugin capabilities out of execution modes that intentionally expose a narrower tool set.

## Trust and administration

Everything below `$CORTEX_HOME/plugins/` is administrator-installed code. Assign only packages whose source and contents you trust: a stdio MCP server can execute local code with the backend process's account, and a remote MCP server can receive tool data over the network. Portable skills are copied into private snapshots, but stdio commands, arguments, working directories, and package data remain trusted administrator-controlled runtime inputs rather than a sandbox boundary. Manifest validation, path containment, private runtime files, redirect rejection, and assignment confirmation reduce configuration mistakes and secret exposure; they do not sandbox plugin behavior.
...
| Settings plugin package manager | `web/src/features/settings/panels/PluginsPanel.tsx` |
...
| Settings skill and MCP editors | `web/src/features/settings/panels/PluginSkillsTab.tsx`, `web/src/features/settings/panels/PluginMcpTab.tsx` |
| Settings assignment and MCP acknowledgment | `web/src/features/settings/panels/PluginAssignPanel.tsx` |
