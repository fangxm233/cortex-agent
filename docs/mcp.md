...
A legacy plugin directory is still passed through to its backend. Claude can load a Claude-native root `.mcp.json` from such a directory, but Cortex does not inventory, summarize, or acknowledgment-gate those native servers, and PI does not receive them. The guarantees in this section apply to portable root `mcp.json` only (`agent-server/src/domain/plugins/runtime.ts:427-434`; `agent-server/src/agent-adapter/claude/spawn-args.ts:224-238`).

Cortex validates and normalizes the package once at spawn time. Claude receives a private supplemental MCP configuration layered after the normal Cortex configuration files. PI receives a private content-addressed configuration consumed by its MCP bridge. PI isolates connection, tool registration, retry, and shutdown by server, so one failed plugin server does not remove bundled Cortex tools or healthy plugin siblings (`agent-server/src/agent-adapter/claude/mcp-config.ts:35-119`; `agent-server/src/agent-adapter/pi/mcp-config.ts:32-256`; `agent-server/src/agent-adapter/pi/mcp-bridge.ts:303-504`).

Portable MCP is omitted when the resolved MCP composition is `none` or `benchmark-thread-run`, and it is not exposed to restricted PI `Agent` subagents. Normal top-level Claude and PI sessions receive it only through an assigned plugin (`agent-server/src/domain/plugins/runtime.ts:117-119,529-559`; `agent-server/src/agent-adapter/pi/adapter.ts:901-918`; `agent-server/src/agent-adapter/pi/mcp-bridge.ts:220-244`).
