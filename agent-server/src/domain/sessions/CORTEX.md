...
| session-registry.ts | adapter | maps short session names to JSONL registry ids |
| session-backup.ts | util | snapshots and restores recorded transcript paths |
| session-hooks.ts | core | dispatches session hooks, diagnostics and injection; injected turns run through startRun on their preserved pool key; owns the onMessageEnd-for-a-finished-turn wrapper |
| session-lifecycle.ts | core | registers, attaches, adopts and resets sessions |
| session-retention.ts | core | sweeps expired bundles and orphan transcripts |
