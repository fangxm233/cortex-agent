...
| `agents/` | [S11] DONE | Agent execution facade, bridge exports, gateway routing, and fallback policy. The normalized callback seam dispatches backend-neutral context usage, tool ids/results, progress, API/compaction notices, plus Web-chat terminal/fallback/session-reset notices. `profile-switch.ts` owns the shared same-backend switch rule |
