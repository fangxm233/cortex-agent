...
| `agents/` | [S11] DONE | Agent execution facade (config.ts + facade.ts), bridge exports, gateway routing, and bounded transient-error fallback policy. The normalized callback seam classifies leading `API Error:` blocks as typed error notices, preserves compaction notices, every tool-use id, and complete tool results through foreground and background-continuation turns. `profile-switch.ts` owns the shared same-backend switch rule consumed by Slack/Feishu `!profile` and Web `sessions.setProfile` |
