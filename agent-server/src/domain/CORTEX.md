...
| `agents/` | [S11] DONE | Agent execution facade (config.ts + facade.ts), bridge exports, gateway routing, and bounded transient-error fallback policy. The normalized callback seam preserves every tool-use id plus complete tool results through foreground and background-continuation turns for DEBUG transcript capture. `profile-switch.ts` owns the shared same-backend switch rule consumed by Slack/Feishu `!profile` and Web `sessions.setProfile` |
