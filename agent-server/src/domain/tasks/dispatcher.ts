...
function buildDispatchPrompt(task: any): string {
  ...
  const completion = [
    ...
    '',
    `Verify done_when conditions, then run:`,
    `  cortex-task complete --project ${task.project} --task-id ${task.id} --note "your completion note"`,
    '',
    `If something outside this turn has to finish first (a long run, a build), arm a waitpoint with`,
    `wait_create, mark the task pending, and complete it when the signal wakes you.`,
    '',
