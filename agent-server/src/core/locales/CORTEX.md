...
`lang` (`!lang` command) · `status` (status/lifecycle prefixes + concise rate-limit and semantic chat notices for compaction, fallback, and backend-session reset + `btn.*` labels) · `commands` (all `!command` replies, help text) · `scheduling` (`!schedule` domain replies) · `interactions` (plan approvals, update prompt, modal/`interactive-builder` labels) · `startup` (startup-notify) · `init` (cortex init wizard + config output).
