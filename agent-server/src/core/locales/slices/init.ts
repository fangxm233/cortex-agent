...
export const initEn = {
  // ── Provider login CLI / init onboarding (protocol values stay untranslated) ──
  'init.auth.loginDescription': 'Interactive provider login (PI bundled; Claude installed only on confirmation)',
  'init.auth.help': 'Login interactively; credentials are detected locally, not inference-tested.',
  'init.auth.backendOption': 'Backend (default: select; PI is bundled)',
  'init.auth.providerOption': 'Provider (default: select from runtime)',
  'init.auth.typeOption': 'Authentication (default: select supported type)',
  'init.auth.exampleProvider': 'Select a provider',
  'init.auth.exampleClaude': 'Claude subscription',
  'init.auth.invalidOptions': 'Invalid login options. Use cortex auth login --help; keys/codes must not be command arguments.',
  'init.auth.invalidBackend': 'Invalid --backend. Valid values: pi, claude.',
  'init.auth.invalidType': 'Invalid --auth-type. Valid values: oauth, api_key.',
  'init.auth.backend': 'Backend',
  'init.auth.bundled': 'PI · bundled',
  'init.auth.provider': 'Provider',
  'init.auth.type': 'Authentication',
  'init.auth.oauth': 'OAuth (subscription login)',
  'init.auth.api_key': 'API key',
  'init.auth.unavailable': 'Provider unavailable. Run cortex auth status or select a runtime-supported provider.',
  'init.auth.unsupported': 'Unsupported authentication. Valid values: ${values}.',
  'init.auth.installDeclined': 'Claude Code installation declined. Run cortex auth login to retry.',
  'init.auth.cancelled': 'Login cancelled. Run cortex auth login to retry.',
  'init.auth.expired': 'Login expired. Run cortex auth login to retry.',
  'init.auth.failed': 'Authentication failed. Run cortex auth status, then cortex auth login to retry.',
  'init.auth.ttyRequired': 'Login requires an interactive terminal. Run cortex auth login in a TTY, or use the Accounts page.',
  'init.auth.noCapabilities': 'No login capabilities available. Run cortex auth status to diagnose.',
  'init.auth.openBrowser': 'Open in your browser: ${url}',
  'init.auth.deviceCode': 'Open ${url} and enter ${code}',
  'init.auth.installPrompt': 'Claude Code is missing. Install @anthropic-ai/claude-code with npm and login?',
  'init.auth.installFailed': 'Claude Code installation failed. Check npm permissions/network, then retry cortex auth login --backend claude.',
  'init.auth.pathMissing': 'Claude Code is still unavailable on PATH. Fix PATH, then retry cortex auth login --backend claude.',
  'init.auth.detectionFailed': 'Credential detection failed; rescan or skip.',
  'init.auth.setup': 'Provider setup (local credentials, not inference verification)',
  'init.auth.continue': 'Continue with configured providers',
  'init.auth.login': 'Login / install and login Claude Code',
  'init.auth.rescan': 'Rescan credentials',
  'init.auth.skip': 'Configure later (Agent may be unavailable)',
  'init.auth.saved': 'Credentials saved. Gateway/profile setup follows; no inference request was made.',
  'init.auth.piFirst': 'PI is bundled. Add providers or install Claude Code in the login step.',
...
export const initZh: Record<keyof typeof initEn, string> = {
  // ── Provider login CLI / init onboarding ──
  'init.auth.loginDescription': '交互式服务商登录（内置 PI；仅在确认后安装 Claude）',
  'init.auth.help': '交互式登录；仅检测本地凭据，不发送推理请求进行验证。',
  'init.auth.backendOption': '后端（默认：交互选择；PI 已内置）',
  'init.auth.providerOption': '服务商（默认：从运行时支持列表中选择）',
  'init.auth.typeOption': '认证方式（默认：选择支持的方式）',
  'init.auth.exampleProvider': '选择服务商',
  'init.auth.exampleClaude': 'Claude 订阅',
  'init.auth.invalidOptions': '登录参数无效。请运行 cortex auth login --help；密钥或授权码不得作为命令参数。',
  'init.auth.invalidBackend': '--backend 无效。可用值：pi, claude。',
  'init.auth.invalidType': '--auth-type 无效。可用值：oauth, api_key。',
  'init.auth.backend': '后端',
  'init.auth.bundled': 'PI · 已内置',
  'init.auth.provider': '服务商',
  'init.auth.type': '认证方式',
  'init.auth.oauth': 'OAuth（订阅登录）',
  'init.auth.api_key': 'API 密钥',
  'init.auth.unavailable': '服务商不可用。请运行 cortex auth status，或选择运行时支持的服务商。',
  'init.auth.unsupported': '不支持此认证方式。可用值：${values}。',
  'init.auth.installDeclined': '已拒绝安装 Claude Code。请运行 cortex auth login 重试。',
  'init.auth.cancelled': '登录已取消。请运行 cortex auth login 重试。',
  'init.auth.expired': '登录已超时。请运行 cortex auth login 重试。',
  'init.auth.failed': '认证失败。请运行 cortex auth status 检查，然后运行 cortex auth login 重试。',
  'init.auth.ttyRequired': '登录需要交互式终端。请在 TTY 中运行 cortex auth login，或使用账号页面。',
  'init.auth.noCapabilities': '没有可用的登录方式。请运行 cortex auth status 检查。',
  'init.auth.openBrowser': '请在浏览器中打开：${url}',
  'init.auth.deviceCode': '请打开 ${url} 并输入 ${code}',
  'init.auth.installPrompt': '未找到 Claude Code。是否通过 npm 安装 @anthropic-ai/claude-code 并登录？',
  'init.auth.installFailed': 'Claude Code 安装失败。请检查 npm 权限和网络，然后运行 cortex auth login --backend claude 重试。',
  'init.auth.pathMissing': 'PATH 中仍找不到 Claude Code。请修复 PATH，然后运行 cortex auth login --backend claude 重试。',
  'init.auth.detectionFailed': '凭据检测失败；请重新扫描或跳过。',
  'init.auth.setup': '服务商设置（检测本地凭据，不验证推理请求）',
  'init.auth.continue': '使用已配置的服务商继续',
  'init.auth.login': '登录 / 安装并登录 Claude Code',
  'init.auth.rescan': '重新扫描凭据',
  'init.auth.skip': '稍后配置（Agent 可能不可用）',
  'init.auth.saved': '凭据已保存。接下来配置网关和模型；未发送推理请求。',
  'init.auth.piFirst': 'PI 已内置。请在登录步骤中添加服务商或安装 Claude Code。',
