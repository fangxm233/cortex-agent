 ...
 * agent-loop boundary, which can be seconds later (K-010 §12 measured a 6s gap). The CLI's
 ...
 *             that turn emits ONE result with its turn count incremented (K-010 §8).
 *   - false — consumed only after the running turn's result, so the CLI starts a turn of its own;
 *             its reply arrives via {@link ContinuationSink} (K-010 §9).
 */
