...
class ClaudeSession {
   ...
   *  order. Each is popped by its `--replay-user-messages` echo (the delivery ack, K-010 §12). */
   ...
   *  a turn of its own for it (K-010 §9). Consumed by the next assistant line, which opens the
  ...
  /**
   * Deliver a user message into the turn already in flight (contract §3, K-010 §8/§9).
   *
   ...
   *   - tool-result boundary → folds into the running turn, ONE result (K-010 §8). Nothing extra
   ...
   *     starts a turn of its own (K-010 §9). The echo handler arms the existing spontaneous-turn
  ...
  private handleReplayEcho(data: any): void {
    ...
    // Consumed with no turn in flight ⇒ this is the K-010 §9 outcome: the CLI is starting a turn
  ...
  private handleLine(line: string) {
    ...
    try {
      ...
      // `--replay-user-messages` echo: the CLI's delivery ack for an injected message (K-010 §12).
      ...
      // (K-010 §9). Both make the CLI open a turn of its own — open a synthetic turn for it so its
