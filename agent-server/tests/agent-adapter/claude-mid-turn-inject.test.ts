...
// output: spec for injectUserMessage + --replay-user-messages ack + both K-010 §8/§9 landing outcomes
...
// --- Real CLI line shapes (K-010 §3 table + probe harness cc-stream-test.mjs) ---
...
// --- K-010 §8: injected message lands on a tool-result boundary → folds into the running turn ---

test('K-010 §8 fold-in: echo while the turn is live acks as folded, opens no continuation, single result', (t) => {
  const { s } = sessionWithStdin(t);
  const cap: { value?: any; resolves?: number } = {};
  s.currentTurn = fakeTurn(cap);
  ...
  assert.deepEqual(acks, [], 'no ack at write time — the CLI may queue it for seconds (K-010 §12)');
...
// --- K-010 §9: injected message lands mid-text-generation → consumed AFTER this turn's result ---

test('K-010 §9 post-result: echo after the result opens a spontaneous turn routed to the continuation sink', (t) => {
  const { s } = sessionWithStdin(t);
  const cap: { value?: any; resolves?: number } = {};
  s.currentTurn = fakeTurn(cap);
  ...
  s.handleLine(RESULT_OK);                        // the model finished its text first (K-010 §9)
...
test('K-010 §9: content-block echo shape is recognised the same as string content', (t) => {
  const { s } = sessionWithStdin(t);
  s.currentTurn = fakeTurn({});
  const acks: any[] = [];
  s.setInjectionAckSink({ onDelivered: (m: any) => acks.push(m) });
