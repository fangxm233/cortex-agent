...
| `pi-mid-turn-inject.test.ts` | Test | PI RPC prompt-steering contract: idle/switch-in-flight/write guards; attachment-aware `streamingBehavior:"steer"` frame; opening-user exclusion; transformed/duplicate FIFO delivery; back-to-back (`all`) consumption; later duplicate rejection ordering; idle-boundary first `agent_end` suppression + aggregate cost/turn result; RPC rejection and process-exit undelivered seals |
