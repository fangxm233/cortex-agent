...
| _vitest-setup.ts | setup | isolates the data home and the Claude config dir per test file |
...
| auth-expiry-persistence.e2e.test.ts | e2e | saved Claude expiry reaches daily warning scan |
...
| local-ui.test.ts | test | local Web UI endpoint enablement and idempotency |
