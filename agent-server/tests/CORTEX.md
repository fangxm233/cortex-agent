...
|---|---|---|
| _shared-pool-manifest.ts | setup | lists tests safe for the shared Vitest pool |
...
| agent-adapter-pi-hook-registry.test.ts | test | covers PI hooks, interactions, and task guards |
...
| hook-exec.test.ts | test | covers bounded hook subprocess execution |
