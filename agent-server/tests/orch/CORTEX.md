...
| `session-events.test.ts` | Test | publishSessionMessage shape + no-bus no-op + the `blockId` that ties a message to its streamed deltas (omitted entirely when nothing streamed); publishSessionMessageDelta shape (blockId / increment / per-block seq) |
