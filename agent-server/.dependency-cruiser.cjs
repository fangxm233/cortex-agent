...
const BENCHMARK_ISOLATION_STAGED = [
  ...
  },
  {
    ...
    severity: 'ignore',
    comment: '§7.3 X2: the composite path may not reach platform adapters (a reachable PlatformAdapter is a live message-sending handle). STAGED at 196 violations = 7 benchmark seeds x 28 platform modules. §18 (18.10) predicted 168 (6 seeds); the seventh seed is Gate 5\'s own composite-runtime-ports.ts, which §7.2 requires to name RunThreadOptions/ThreadRecord/ThreadTemplate (P9, P10) and the shipped WorkspaceLease (P18) — both reach @platform/index.js. NO NEW ROUTE: the new seed inherits the same L0 coupling as the other six. Route module outside Gate 5: src/core/types/thread-types.ts:476 type-re-exports PlatformAdapter/MessageRef/Destination from @platform/index.js — an L0 module used repo-wide; second route domain/costs/rate-limit-throttle.ts:6. Owner: no gate of this plan (escalated as §18 G5-N1)',
    from: BENCHMARK_FROM,
