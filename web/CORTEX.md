...
| `src/mobile/` | Mobile v3 shell: native full-bleed viewport, four-tab navigation, animated drill-in routes, shared mobile UI kit, and real tRPC-backed screens. The obsolete mock-device frame and v2 container screens were removed; a few pure v2 helpers remain where v3 imports them. See its CORTEX.md. |
| `src/i18n/` | Real, **user-controlled + persisted** language (`useLang`/`useSetLang`, localStorage `cortex.lang`, default en / zh from navigator) — the switch now lives in the Settings **Appearance** panel (EN/中 segmented control, next to the theme toggle); decoupled from viewport. `useIsMobile` now reflects the mobile client shell (`isMobileShell`), not width. `useVocab` → the active-language vocab, including desktop DEBUG inspector labels and character-count units. See `lang.ts`/`vocab.ts`/`LangProvider.tsx`. |
...
| `src/shell/` | `AppShell` renders `<Outlet/>` and keeps global overlays mounted, including Command Palette and Settings; `/workbench` owns its frame and rail. |
| `src/features/workbench/` | **App-shell frame + 22a dual-zone Left Rail + Center Chat + Right Panel** — shares context bar/details; desktop uses a modal and mobile a bottom sheet. `LeftRail` uses the dual-zone projects/sessions scheme with live attention state. See its CORTEX.md |
...
| `src/features/live/` | **The app's single live-event stream** — one shared SSE subscription carrying context plus content-free config/provider-throttle hints, fanned out client-side with scope parity and reconnect recovery. See its CORTEX.md |
| `src/features/rate-limit/` | Active-only provider/window throttle status: authoritative query + shared SSE invalidation + local countdown VM; desktop popover and mobile Projects bottom sheet. See its CORTEX.md. |
...
| `src/features/command-palette/` | ⌘K command palette over real sessions/threads/tasks; page targets navigate and Settings opens as an in-place modal. See its CORTEX.md |
...
| `src/features/settings/` | **Settings modal 12a–g 1:1** — `SettingsProvider` opens one Radix Dialog in place without route changes or workbench remount; real `config.get`, with live Budget write. See its CORTEX.md |
...
- `build` = `tsc --noEmit && vite build`; `typecheck` = `tsc --noEmit`; `test` = `vitest run`. The maintained suite emphasizes pure state/VM logic, routing and action availability, persistence, and protocol boundaries. Pixel/color/copy snapshots, source-shape smoke tests, and broad static-render inventories are intentionally excluded.
- Tailwind pinned to v3.4 (config-file token contract; v4 moves tokens to CSS `@theme`).
- Tabs/Tooltip use `@radix-ui/react-tabs` / `@radix-ui/react-tooltip` (approved primitive layer, design §1); token-only styling. Task status rendering imports `design/StatusPill` directly; the unused `features/tasks/Pills.tsx` wrapper was removed.
