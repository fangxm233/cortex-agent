...
| `src/features/live/` | **The app's single live-event stream** — one shared SSE subscription carrying the fixed event union (including `session.context-usage`), fanned out client-side with scope parity and reconnect recovery. See its CORTEX.md |
