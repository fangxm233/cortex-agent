...
| 1b 会话详情 (+1m/1n/1o/1p) | `MChatScreen`, `MChatView`, `m-chat-vm` | `sessions.transcript`+`sessions.send`/`createAndSend`+live; profile switch (`sessions.setProfile`), attachments upload. **Unread write side:** mounts the desktop `useMarkSessionRead(sessionId, `liveTail.length:running`)` — viewing stamps the session read (debounced, visibility-gated, re-arming on live activity) → invalidates `sessions.list` so the 1a marker + 1e switcher badge clear. **1m 提问 / 1n Plan 审批: UI built + tested but NOT wired — the web tRPC contract has no session-scoped interaction/plan stream (documented gap; no fabrication).** |
