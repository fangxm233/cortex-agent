...
| 1e 项目 | `MProjectScreen/View`, `m-project-vm` | `projects.list`+`cost.summary`+`threads.list`+`approvals.list`+`machines.list`; switch project; →memory/machines/settings/approvals/new-project. **Unread-first ordering (parity with desktop ProjectMenu):** an UNSCOPED `sessions.list`(direct) → `unreadCountByProject` (reused from `features/workbench/project-menu`) badges each switch row with its unread count (accent pill) and floats projects-with-unread first (`buildProjectSwitchRows` stable sort); kept fresh by `useSessionsLiveSync`. |
