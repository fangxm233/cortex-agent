...
| `MobileShell.tsx` | Frame owner: full-bleed `100dvh` flex column + `BottomTabBar` (only on Tab routes) + scroll `<Outlet/>`, wrapped in `MobileProjectProvider`; mounts the global `MNotificationProvider` (1q). Binds real `approvals.list` → 需要你 badge. |
