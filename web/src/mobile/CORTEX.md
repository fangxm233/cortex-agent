...
| `screens/MobileSessionsScreen.tsx` | **Session screen 5a** (task c880) — 1:1 rebuild from `scheme.dc.html` L2932-3003: session header (QN avatar/title/`running·turns·$`) + chat stream (dark user bubble / collapsed tool chips / assistant + inline experiment-pipeline stepper card) + composer (input + running status line + send). **No inline approval card** (see the presentational row below — approvals aren't session-scoped). `running` is snapshot + delta (2026-07-14 fix): `SessionInfo.running` + the live `session.status` event via the shared `useSessionMessageLiveSync`, same rule as desktop CenterChat (was the streaming heuristic only). The header `turns` is the **REAL agent-turn count** (grows as the agent works, not user-message rounds): `resolveTurns(liveTurns, SessionInfo.numTurns)` — `session.turn` delta > snapshot > `—`, same as desktop. Real tRPC: `sessions.transcript` (chat) · `threads.get` (inline card) · `sessions.send` (send). Missing fields (session cost/elapsed) → explicit `—`, never fabricated. The bottom Tab is the shell's (not re-rendered). |
| `screens/{MobileSessionHeader,MobileMessageStream,MobileThreadStepper,MobileApprovalCard,MobileComposer}.tsx` | 5a presentational + wired parts (`MobileInlineThreadCard`/`MobileApprovalCardContainer` bind real tRPC). Pure `mobile-session-vm.ts` maps DTOs → the scheme slot model (initials/status-line/ZH divider/horizontal stepper/approval desc/tool chips). `MobileMessageStream`: assistant text renders **rich Markdown** via the shared `features/workbench/ChatMarkdown` (+ a `cxblink` streaming caret); the tool-calls row is **tap-to-expand** (collapsed chips → per-call kind+input, mirrors desktop `ToolCallsRow`). **No inline approval card** — `ApprovalInfo` has no session link, so an inline card would show the first GLOBAL pending approval in every session; approvals live on the 审批 screen (10e) + the 会话 Tab amber dot. `MobileApprovalCard{,Container}.tsx` are kept (still render-tested) but no longer mounted by the session screen — mirrors the desktop legacy ApprovalCard. |
