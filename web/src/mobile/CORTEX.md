...
| 1b 会话详情 (+1m/1n/1o/1p) | `MChatScreen`, `MChatView`, `m-chat-vm` | `sessions.transcript`+`sessions.send`/`createAndSend`+live; profile switch (`sessions.setProfile`), attachments upload. **Inline thread card (1b L148-158):** `InlineThreadCard` renders below the transcript, bound to REAL `threads.get` but **scoped to THIS session** — `threads.list({status:['running','waiting'], sessionId})` resolves the session→channel server-side and returns only threads on it, so the card shows the thread this conversation spawned (never a random global one, the former "wrong time" bug); hidden on drafts (no `sessionId`) and when the session owns no active thread. `打开 →` drills to 1g. **Stop (running):** while the session is running the composer's Send button swaps for a Stop button (white square on ink, `MComposer running`/`onStop`) → `sessions.cancel({sessionId})` cancels the agent(s) on the channel (same path as the desktop Composer); `running` collapses to idle as the live stream quiets. **Unread write side:** mounts the desktop `useMarkSessionRead(sessionId, `liveTail.length:running`)` — viewing stamps the session read (debounced, visibility-gated, re-arming on live activity) → invalidates `sessions.list` so the 1a marker + 1e switcher badge clear. **1m 提问 / 1n Plan 审批: UI built + tested but NOT wired — the web tRPC contract has no session-scoped interaction/plan stream (documented gap; no fabrication).** |
