...
function QuickNoteInput({ copy, busy, onAdd }: { copy: NotesCopy; busy: boolean; onAdd: (text: string) => Promise<unknown> }) {
  ...
  return (
    <form onSubmit={submit} onClick={(event) => event.stopPropagation()} style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 9, border: `1px solid ${MC.divider}`, borderRadius: 'var(--r-chip)', padding: '6px 10px' }}>
  ...
  return (
    <div data-mobile-notes-card="">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ font: `600 10px ${MONO}`, color: MC.sub, background: 'var(--proto-line-2)', padding: '2px 8px', borderRadius: 'var(--r-pill)' }}>{vm.activeCount}</span>
