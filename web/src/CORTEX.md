...
Target structure, enforced import rules and the violation baseline: `web/CORTEX.md`.
...
| index.css | type | Tailwind layers, the 16 verbatim `cx*` prototype keyframes, SPA-wide structure over the `public/theme.css` tokens |
...
|---|---|---|
| `lib/` | Bottom of the stack: platform shell, tRPC transport, session, pure helpers | `lib/CORTEX.md` |
| `design/` | Primitive kit + the two app-free seams (modal registry, dock intake) | `design/CORTEX.md` |
| `theme/` | Runtime appearance: theme, accent, palette, `ThemeProvider` | `theme/CORTEX.md` |
| `i18n/` | Bilingual vocab tables + `LangProvider` / `useVocab` | `i18n/CORTEX.md` |
| `features/` | 33 feature directories — the shared body both chromes render | `features/CORTEX.md` |
| `shell/` | Desktop chrome: frame, top bar, menus, provider/modal composition | `shell/CORTEX.md` |
| `mobile/` | Mobile chrome: routes, tabs, screens, mobile kit | `mobile/CORTEX.md` |
| `dev/` | DEV-only demo routes, absent from production bundles | `dev/CORTEX.md` |

Sub-indexes also exist for the three large features: `features/session/CORTEX.md`,
`features/workbench/CORTEX.md`, `features/settings/CORTEX.md`.
...
providers.tsx          query client · tRPC · theme · tooltip · toast · vocab · login gate
 └ shell/ShellProviders   mounted by EACH chrome, never by the root: live stream ·
     ...
     └ per-chrome         AppShell: dock, selected session, nav history, pane state, notes
                          MobileShell: its two headless mounts
