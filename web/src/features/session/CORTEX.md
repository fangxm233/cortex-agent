...
The session/chat core both chromes render, lifted out of `workbench/` in step 2 (88 files).
...
| dir | what goes there |
|---|---|
| `transcript/` | Rendering a message stream: rows, tool calls, subagents, notices, attachments, edit, optimistic and reveal pacing |
...
- **transcript/** (27 files) — `MessageStream`, `ToolCallsRow` + `tool-call-overflow` +
...
- **list/** — `session-groups`, `session-id`, `session-run-status`, `session-stats`,
  `left-rail-projects`, `rail`/`selection`/`project`/`profile` menus, `schedule-rail`,
  `scheduled-chat`.
- **live/** — `useSessionMessageLiveSync`, `useSessionsLiveSync`, `useThreadsLiveSync`,
  ...
  `useMarkSessionRead`.
- **rail/** — `TodoRail` + `todo-vm`, `WaitRail` + `wait-rail-vm`.
