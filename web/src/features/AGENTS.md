...
2. **No bidirectional pair** of `features/<a>` ↔ `features/<b>`, checked by
   `scripts/check-feature-cycles.mjs` over 61 inter-feature edges. One allow-listed pair
   remains: `settings↔usage` (`scripts/feature-cycles-allowlist.json`). Two features that
   import each other are one feature, or one of them is missing a seam. CSS imports count:
   a stylesheet several features share (`design/content-surfaces.css`) lives in `design/`.
...
| `session/` | The session/chat core both chromes render (92 files) — see `session/AGENTS.md` |
| `settings/` | The settings modal and every panel (79 files) — see `settings/AGENTS.md` |
...
| `workbench/` | The desktop three-pane page only (49 files) — see `workbench/AGENTS.md` |
