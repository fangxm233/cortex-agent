...
|---|---|---|
| SettingsModal.tsx | entry | The glass modal shell: grouped nav column, header action slot, the selected section's panel, dirty-navigation guards |
| useSettings.tsx | entry | The registry key and `useSettings()` / `SettingsModalHost`, mounted by `shell/ShellModals`; loads the portal styles |
| settings-nav.ts | core | `SettingsSectionKey` and the grouped nav model (`getSettingsNavGroups(L)`) |
