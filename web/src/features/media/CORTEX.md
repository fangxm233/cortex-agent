...
| `DocViewer.tsx` | `DocViewerProvider` + `useDocViewer()` (`openDoc(item)` / `close()`) + the `DocModal`. `DocItem = { kind:'pdf'\|'text', name, path, mimeType? }` — bytes auth-fetched from `/api/files/download`. **text**: `.md` → `ChatMarkdown`, else monospace `<pre>` (2 MB preview cap → "download to view"). **pdf**: pdf.js renders each page to a `<canvas>` — reliable in webkit2gtk / Android System WebView, which have **NO built-in PDF viewer** (an `<iframe src=blob:pdf>` would silently blank there). Header ↓ download / × close, Esc / scrim close, body-scroll lock. No-op default context like MediaViewer. |
