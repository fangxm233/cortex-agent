...
| `thread-detail-vm.test.ts` | Unit tests for header/state derivation, step/session attribution, drillability, artifacts, and terminal behavior; palette values are not asserted. |
...
| `thread-steps.ts` | Pure `selectActiveStep` and `dispatchesForStep`; exact `stepIndex` keeps reused agent slots from merging stages. Unconsumed summary/child helpers were removed. |
| `thread-steps.test.ts` | Exact step-index dispatch-attribution regression. |
| `nested-threads.ts` | Pure `nodeLevel`, `isMaxLevel`, `countDescendants`, and clamped `treeMaxLevel`, consumed by thread detail and workbench right-panel models. Unconsumed outline/visibility helpers were removed. |
| `nested-threads.test.ts` | Unit tests for node-level and bounded tree-depth calculations. |
