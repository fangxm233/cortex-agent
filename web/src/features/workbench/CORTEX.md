...
| `project-menu.ts` / `project-menu.test.ts` | **Pure** (TDD, 12 tests): `runningCountByProject` (threads.list running+waiting grouped by projectId) · `unreadCountByProject` (unscoped sessions.list unread grouped by projectId) · `buildSwitchList` (projects minus active + real counts; projects with UNREAD sessions sort first, stable within halves; row carries `unread` count) · `switchRowMeta` · `projMenuSubLabel` (`N threads running · $X today`). |
