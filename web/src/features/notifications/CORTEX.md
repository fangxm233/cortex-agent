...
| path | role |
|---|---|
| `notification-vm.ts` | Pure view-model: `NotificationItem`/`NotificationLevel`, preview shaping, DM/system notice construction, and `isTransient` (info auto-dismisses; warning/error remain resident). Unit-tested without asserting a duplicated timing constant. |
...
| `useDmNotifications.ts` | Thin glue over the SHARED live stream (`features/live`): global `session.message` + `session.status` (no sessionId scope); non-empty `assistant` messages → `onMessage` (buffered), `session.status` running:false → `onTurnEnd` (turn boundary → one toast). Turn-scoped so a multi-message turn produces a single toast, not a burst. |
...
| `useSystemNotices.ts` | Thin glue over the SHARED live stream: `system.notice`; filters to non-empty events and hands each (`level`/`text`/`title`/`ts`) to `onNotice`. No membership gate — every system broadcast surfaces. Mirrors `useDmNotifications`. |
...
- `tailwind.config.ts` owns the visual 6-second progress animation for transient info toasts; warning/error toasts remain resident. The duration is not exported as a duplicate view-model contract.
