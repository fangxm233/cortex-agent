...
| `open_path` | `(path) → Result<()>` | **Desktop** download-complete toast "Open file" action (`web/src/features/media/useDownloadFile.ts` → `web/src/lib/files.openPath`): opens the saved file with the OS default app. Fire-and-forget `std::process::Command` per platform — Linux `xdg-open`, macOS `open`, Windows `cmd /C start "" <path>`. Args passed directly (no shell → no path-as-command injection); the opener's exit code is ignored (Explorer returns non-zero on success). Android branch returns an error (never invoked there — mobile uses the DownloadManager notification). |
| `reveal_path` | `(path) → Result<()>` | **Desktop** download-complete toast "Open folder" action (`web/src/lib/files.revealPath`): reveals the saved file in the OS file manager. Linux opens the containing folder (`xdg-open <parent>` — no portable select), macOS `open -R` (reveal + select in Finder), Windows `explorer /select,<path>` (folder with the file highlighted). Same no-shell / desktop-only shape as `open_path`. |
