...
| `web/` | Vite + React SPA; browser-responsive desktop/mobile layouts share one router. Built to `web/dist`, served by the in-core UI host and native shells. Structure map + enforced import rules: `web/CORTEX.md`. |
...
| `data/provider-state.json` | Provider usage, throttle windows and provider-attributed resume queue |
