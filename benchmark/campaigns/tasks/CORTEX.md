Update this file whenever this directory changes

Harbor task directories include synthetic ZERO-PAID fixtures and authentic Terminal-Bench 2.1
trial copies. A task directory is mounted into the trial container, so no index file is placed inside one.

| filename | role | function |
|---|---|---|
| terminal-bench-2.1/ | tasks | Holds three digest-pinned authentic benchmark tasks |
| zero-paid-one/ | task | Asks for one literal file and verifies it offline |
| zero-paid-two/ | task | Asks for a second literal file so the campaign runs two trials |
| zero-paid-refused/ | task | Asks in a shape the synthetic model refuses, so the agent run fails and the verifier scores it 0 |
