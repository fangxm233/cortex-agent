...
| trial_admission.py | boundary | Seals the agent phase's launch inputs, admits what a pinned image declares for itself, proves the container kept its CPU pin, records host load at both ends, and discards trial scratch |
...
| trial_verifier.py | boundary | Scores the task in the environment its image ships, not the one sealed for the agent |
