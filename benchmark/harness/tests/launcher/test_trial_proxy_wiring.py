...
from trial_proxy_fixtures import (
    ...
    recording_session, seed_credential, trial_seed,
)
...
@pytest.fixture(autouse=True)
def cleanup_proxy_listeners(
    request: pytest.FixtureRequest, monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Always stop real listeners, including when an earlier assertion or setup fails."""
    from cortex_bench_harness.launcher import trial_proxy

    start = trial_proxy.start_trial_proxy
    ...
    monkeypatch.setattr(trial_proxy, "start_trial_proxy", tracked_start)
...
class BrokenContainerEnvironment(ContainerEnvironment):
    """A mis-installed image fails the real bootstrap-file check after its route is armed."""

    async def exec(self, command: str, **kwargs: object) -> ExecResult:
        if f"test -f {BUNDLE_ROOT}/dist/entry/production-app-bootstrap.js" in command:
            self.calls.append(command)
            return ExecResult(stdout="", stderr="bootstrap file missing", return_code=1)
...
def test_public_entry_revokes_the_route_when_install_fails(
    ...
    with pytest.raises(RuntimeError, match="bootstrap file missing"):
