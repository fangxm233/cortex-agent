...
#
# Every state below is test-local, in both directions. What is under proof is the mechanism — an
# unadmitted state stops the route before a credential is read, whichever row carries it — and not
# the shipped state of any row. A row's state is a determination that moves as its evidence
# arrives, so an assertion here about a shipped value breaks this file on the next raise while
...
def test_arming_refuses_a_capability_row_no_authority_admits(
    ...
    """The row's key carries no `??` and has a registered adapter, so adapter selection admits it.
    ...
    assert ADAPTED_ROW in str(refusal.value)
    assert "unsupported" in str(refusal.value)


def test_the_refusal_precedes_the_host_credential_read(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_a_refused_row_leaves_no_route_and_no_artifact(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    refuse_capability(monkeypatch, ADAPTED_ROW)
...
def test_the_public_entry_refuses_before_the_container_could_exist(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
